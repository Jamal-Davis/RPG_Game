...
//	public:
		Item();
		Item(string, int, int, int, bool, bool, bool);
		void set (string, int, int, int, bool, bool, bool);
		void Use();		
};
Item::Item(){
	name = "NULL";stock = 0;MAX_stock = 0;id = -1;
...
void Item::set (string NA, int ST, int M_ST, int ID, bool CON, bool EQP, bool KEY) {
        name = NA;stock = ST;MAX_stock = M_ST;id = ID;
        consum = CON;equip = EQP;key=KEY;
} 
void Item::Use() {
	if (id == -1)
		...
		if (stock < 0)
			{
			stock = 0;
			cout << "Out of " << name << ", cannot use." << endl;
			}
		}
}
